 ...
 * tileCameraGenerator.algorithm = SpaceFillingAlgorithm.ScanLine;
...
export class TileCameraGenerator {
    ...
    /** @see {@link algorithm} */
    protected _algorithm = SpaceFillingAlgorithm.ScanLine;
    ...
    /**
     * Used for Iterator behavior. It sets the tile to 0 (first tile) if the value of tile is negative and therefore
     * initiates the iteration. Otherwise it increments the tile and calls update to directly change the camera. If the
     * tile index would get out of range it returns false to indicate, that all tiles were rendered.
    ...
    public sourceCameraChanged(): void {
        assert(this._sourceCamera !== undefined, `expected the input/source camera to be defined`);
    ...
    get numXTiles(): number {
        if (this.sourceViewport === undefined) {
            return -1;
        }
    ...
    /**
     * Returns the sourceCamera which viewport should be divided in tiles. If the sourceCamera has not been set, it
    ...
    /**
     * Returns the size of the original viewport which should be divided in tiles based on the tile size. If the
     * viewport has not been set, it returns [-1, -1].
    ...
    /**
     * Sets the size of the viewport from the sourceCamera. It checks if the sourceViewport is compatible with the
     * selected algorithm and eventually adjusts the tileSize to match the conditions of the algorithm.
     */
     ...
     */
    get algorithm(): SpaceFillingAlgorithm {
     ...
     */
    set algorithm(algorithm: SpaceFillingAlgorithm) {
...
export enum SpaceFillingAlgorithm {
    /**
